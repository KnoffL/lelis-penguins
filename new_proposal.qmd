...
- use functions where sensible e.g. to calculate weighted averages
...
- isolate the observations where the variable "Bildung_Casmin_Name" is either high, medium or low and where depressive symptoms are given
...
If a mistake is noticed the partner is informed and the change can be inspected by using the log function and reversed with git restore.

if we notice, that we have to repeat a code snippet a lot we can think of a sensible function to accelerate the analysis.
